...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("POST /api/albums database connection error:", err);
    const message =
...
export async function PUT(request: NextRequest) {
  ...
  const idRaw = bodyObj?.id ?? bodyObj?.albumId;
  const albumId =
  ...
  const { title, artist, year, description, image, tracks } = bodyObj;
  const titleStr = typeof title === "string" ? title.trim() : "";
  const artistStr = typeof artist === "string" ? artist.trim() : "";
  if (!titleStr || !artistStr) {
    return NextResponse.json(
  ...
  try {
    ...
    console.error("PUT /api/albums database connection error:", err);
    const message =
      err instanceof Error ? err.message : "Database connection failed";
    return NextResponse.json({ error: message }, { status: 500 });
  }
